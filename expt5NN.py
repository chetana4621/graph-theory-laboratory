...
# Manual line graph construction
L = nx.Graph()
steps = []
...
print(f"Original graph: {G.number_of_nodes()} vertices, {G.number_of_edges()} edges")
print(f"Line graph: {L.number_of_nodes()} vertices, {L.number_of_edges()} edges")
print("(|V(L(G))| = |E(G)| confirmed)" if L.number_of_nodes()==G.number_of_edges() else "")
